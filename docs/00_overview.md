...
## 3. 企画・要件ドキュメントの現状
- 企画・要件・設計・運用の主要ドキュメントは作成済み。
- `ea/`（EA Runtime）、`gui/`（GUI Builder）、`python/`（Backtestエンジン）の実装あり。
- テスト/運用スクリプトは `scripts/` に集約済み。

## 4. 実装の位置づけ
- `ea/`: EA Runtime MVP を実装済み（テスト設定は `ea/tests/`）。
- `gui/`: GUI Builder MVP を実装済み。
- `python/`: Backtestエンジンを実装済み。
- 重要制約（M1/確定足/同一足禁止）は実装で担保する前提。
...
**A4. Nanpinモードの詳細ルール → 決定：MVPでは発注/管理ロジックは未実装**
- ブロック定義は用意済み（nanpin.fixed等）
- 実際のナンピン発注/管理は Phase 4 で実装
- 理由：MVP範囲の明確化、安全性優先
